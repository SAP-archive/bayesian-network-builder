...
object Collider {
  ...
  def apply[A, B, C](a: BE[A], b: BE[B], cpt: CPT2[A, B, C]): BE[C] = {
    ...
    val bb = b.chances
    require(
      cpt.map(_._1._1).toSet.size == aa.keySet.size,
      "A elements do not match"
    )
    require(
      cpt.map(_._1._2).toSet.size == bb.keySet.size,
      "B elements do not match"
    )
    require(
      cpt.map(_._2.chances.size).toSet.size == 1,
      "BE[C] not fully defined"
    )
    ...
    new BE(
      ch.keySet
        .map(c =>
          (
            c,
            cpt.foldLeft(0d)((acc, cp) =>
              acc + (aa(cp._1._1) * bb(cp._1._2) * cp._2.chances(c))
            )
          )
        )
        .toMap
