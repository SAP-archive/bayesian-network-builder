...
/**
  ...
  */
object Bayes {
  def apply[A, B](a: BE[A], cond: Map[A, BE[B]]): Bayes[A, B] =
    new Bayes[A, B](a, cond)
}
...
/**
  ...
  * @tparam B
  */
class Bayes[A, B](val a: BE[A], val a2b: Map[A, BE[B]])
    extends BE[B](a.chain(a2b).chances) {
  val inv: Map[B, BE[A]] = chances.keySet
    .map(bcase =>
      (
        bcase,
        new BE(
          a.chances.map(av =>
            (
              av._1,
              a2b(av._1).chances(bcase) * a.chances(av._1) / chances(bcase)
            )
          )
        )
      )
