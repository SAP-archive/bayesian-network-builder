...
package object bn {
  ...
  implicit class CPT2Ops[A, B, C](cpt: CPT2[A, B, C]) {
    def from(priorA: BE[A], priorB: BE[B]): BE[C] =
      Collider(priorA, priorB, cpt)
  }
  ...
  class BE[T](val chances: Map[T, Double]) extends Equals {
    require(
      chances.values.forall(_ >= 0),
      "all probabilities must be > 0, " + chances
    )
    require(
      math.abs(1 - chances.values.sum) <= .06,
      s"probabilities must sum up to 1, $chances, sum:${chances.values.sum}"
    )

    override def toString: String =
      chances.toSeq
        .sortBy(_._1)(Ordering.by(_.toString))
        .map(e => s"${e._1}-> .${BE.format(e._2)}")
        .mkString(", ")
  ...
  implicit class BayesOps[A, B](a: BE[A]) {
    ...
    def chain(clauses: Seq[(A, BE[B])])(implicit d: DummyImplicit): BE[B] =
      TotalProb(a, clauses)
  ...
  object BE {
    ...
    def format(n: Double): String =
      "%02d".format(((math floor n * 100) / 100 * 100).toInt)
  }

  object Cards {
    def apply[T](clauses: (T, Double)*) =
      new BE((clauses.map(_._1) zip Norm(clauses.map(_._2))).toMap)
  }
  ...
  object Sure {
    ...
    def apply[T](k: T, all: Set[T]): BE[T] =
      new BE[T](
        all
          .map(s => {
            (
  ...
  object Convert {
    ...
    private def chainRedux[T](tuples: Seq[BE[T]]): BE[T] =
      tuples.reduce(Conflationer(_, _))
